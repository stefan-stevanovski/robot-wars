...
function ActorManager:OnFireWeapon(eventData)
	...
	lookAt.y = lookAt.y + 1.17111111111111111;
	--lookAt.x = lookAt.x - 0.6;
	local dir = lookAt * 1.2;	
	pos = pos + dir;
	...
	local laser_fire = CreateActor("actors\\sphere.xml", pos, ypr);	
	if (laser_fire ~= -1) then
		dir:Normalize();
		ApplyForce(dir, 0.4, laser_fire);
	end
end
function ActorManager:OnDestroyPlayer(eventData) 

end

function ActorManager:_DestroyOnAsteroidCollision(teapot)
	if (teapot == self._player) then
		QueueEvent(EventType.EvtData_Destroy_Player, teapot:GetActorId());
	else
...
function ActorManager:OnPhysicsCollision(eventData)
	...
	if (actorA.actorType == "Asteroid") then
		teapot = actorB;
	elseif (actorB.actorType == "Asteroid") then
		teapot = actorA;
	end
	
	if(teapot ~= nil) then
		self:_DestroyOnAsteroidCollision(teapot);
		return;
	end
	...
	-- play the hit sound
	QueueEvent(EventType.EvtData_PlaySound, "audio\\triger.ogg");
end
...
function ActorManager:UpdateUi()
	...
	local uiText = "You are at " .. self._player.hitPoints .. " HP. Be careful!!!\n";
	if (self._enemies ~= nil) then
		for id, teapot in pairs(self._enemies) do
			uiText = uiText .. "teapot ID: " .. id .. " teapot HP: " .. teapot.hitPoints;
		end
	end
...
function ActorManager:_DamageTeapot(teapot)
	...
	if (teapot.hitPoints <= 0) then
		if (teapot == self._player) then
			QueueEvent(EventType.EvtData_Destroy_Player, self._player:GetActorId());
			return;
		else
			self:RemoveEnemy(teapot);
			QueueEvent(EventType.EvtData_Request_Destroy_Actor, teapot:GetActorId());
