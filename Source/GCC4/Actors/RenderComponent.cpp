    ...
    if (pTransformComponent)
    {
		...
		switch (GameCodeApp::GetRendererImpl())
		{
			case GameCodeApp::Renderer_D3D9: 
				return shared_ptr<SceneNode>(GCC_NEW D3DRobotMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));

			case GameCodeApp::Renderer_D3D11: 
			{
				Mat4x4 rot90;
				rot90.BuildRotationY(GCC_PI/20.0f);
				shared_ptr<SceneNode> parent(GCC_NEW SceneNode(m_pOwner->GetId(), weakThis, RenderPass_Actor, &pTransformComponent->GetTransform()));
				shared_ptr<SceneNode> Robot(GCC_NEW D3DRobotMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90));
				parent->VAddChild(Robot);
				return parent;
			}
								 
			default:
				GCC_ERROR("Unknown Renderer Implementation in RobotRenderComponent");
		}
    }
