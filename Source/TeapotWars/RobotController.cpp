// RobotController.cpp - Controller class for the player Robots
//
...
//
// RobotController::RobotController			- Chapter 19, page 735
//
RobotController::RobotController(shared_ptr<SceneNode> object)
: m_object(object)
{
...
//
// RobotController::VOnLButtonDown				- Chapter 19, page 735
//
bool RobotController::VOnPointerButtonDown(const Point &mousePos, const int radius, const std::string &buttonName)
{
	if (buttonName != "PointerLeft")
		return false;
	ActorId actorId = m_object->VGet()->ActorId();
	GCC_ASSERT(actorId != INVALID_ACTOR_ID && _T("The Robot controller isn't attached to a valid actor!"));
...
//
// RobotController::OnUpdate				- Chapter 19, page 736
//
void RobotController::OnUpdate(DWORD const deltaMilliseconds)  
{
    //
}

bool RobotController::VOnKeyDown(const BYTE c)
{
...
bool RobotController::VOnKeyUp(const BYTE c)
{
    // update the key table
    m_bKey[c] = false;
