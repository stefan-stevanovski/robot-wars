#pragma once
//========================================================================
// RobotWarsView.h : source file for the sample game
//
...
class RobotWarsHumanView : public HumanView
{
protected:
	bool  m_bShowUI;					// If true, it renders the UI control text
    std::wstring m_gameplayText;

	shared_ptr<RobotController> m_pRobotController;
	shared_ptr<MovementController> m_pFreeCameraController;
	shared_ptr<SceneNode> m_pRobot;
	shared_ptr<StandardHUD> m_StandardHUD; 

public:
	RobotWarsHumanView(shared_ptr<IRenderer> renderer); 
	virtual ~RobotWarsHumanView();
...
class AIRobotView : public IGameView 
{
	friend class AIRobotViewListener;
...
public:
	AIRobotView(shared_ptr<PathingGraph> pPathingGraph);
	virtual ~AIRobotView();
