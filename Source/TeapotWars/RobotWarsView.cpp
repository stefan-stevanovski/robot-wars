...
extern RobotWarsApp g_RobotWarsApp;
...
MainMenuUI::MainMenuUI()
{
	...
	D3DCOLOR color = D3DXCOLOR((float)47 / 255, (float)79 / 255, (float)79 / 255, 1.0f);
	m_SampleUI.SetBackgroundColors(color);

	m_SampleUI.SetFont(0, L"Ariel", height, 0);

	m_SampleUI.AddStatic(0, L"Robot Wars Main Menu", iX-20, iY, g_SampleUIWidth, height * 2);
	iY += (lineHeight * 3);
...
{
	HRESULT hr;
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"RobotWarsHUD" ); // These events are to help PIX identify what the code is doing
...
//========================================================================
//
// RobotWarsHumanView Implementation
//
//========================================================================

//
// RobotWarsHumanView::RobotWarsHumanView	- Chapter 19, page 724
//
RobotWarsHumanView::RobotWarsHumanView(shared_ptr<IRenderer> renderer) : HumanView(renderer)
{ 
	m_bShowUI = true; 
...
RobotWarsHumanView::~RobotWarsHumanView() 
{
    RemoveAllDelegates();
}

//
// RobotWarsHumanView::VOnMsgProc				- Chapter 19, page 727
//
LRESULT CALLBACK RobotWarsHumanView::VOnMsgProc( AppMsg msg )
{
	if (HumanView::VOnMsgProc(msg))
		return 1;
	...
	if (msg.m_uMsg==WM_KEYDOWN)
	{
		...
		{
			RobotWarsLogic *twg = static_cast<RobotWarsLogic *>(g_pApp->m_pGame);
			twg->ToggleRenderDiagnostics();

		}
		else if (msg.m_wParam==VK_F9)
		{
			m_KeyboardHandler = m_pRobotController;
			m_PointerHandler = m_pRobotController;
			m_pCamera->SetTarget(m_pRobot);
			m_pRobot->SetAlpha(0.8f);
			ReleaseCapture();
			return 1;
		}
...
//
// RobotWarsHumanView::VRenderText				- Chapter 19, page 727
//
void RobotWarsHumanView::VRenderText()
{
...
//
// RobotWarsHumanView::VOnUpdate				- Chapter 19, page 730
//
void RobotWarsHumanView::VOnUpdate(unsigned long deltaMs)
{
	HumanView::VOnUpdate( deltaMs );
	...
	if (m_pRobotController)
	{
		m_pRobotController->OnUpdate(deltaMs);
	}
...
//
// RobotWarsHumanView::VOnAttach				- Chapter 19, page 731
//
void RobotWarsHumanView::VOnAttach(GameViewId vid, ActorId aid)
{
	HumanView::VOnAttach(vid, aid);
}

bool RobotWarsHumanView::VLoadGameDelegate(TiXmlElement* pLevelData)
{
...
void RobotWarsHumanView::VSetControlledActor(ActorId actorId) 
{ 
	m_pRobot = static_pointer_cast<SceneNode>(m_pScene->FindActor(actorId));
    if (!m_pRobot)
    {
        GCC_ERROR("Invalid Robot");
        return;
    }

	HumanView::VSetControlledActor(actorId);

    m_pRobotController.reset(GCC_NEW RobotController(m_pRobot));
    m_KeyboardHandler = m_pRobotController;
    m_PointerHandler = m_pRobotController;
    m_pCamera->SetTarget(m_pRobot);
    m_pRobot->SetAlpha(0.8f);
...
void RobotWarsHumanView::GameplayUiUpdateDelegate(IEventDataPtr pEventData)
{
...
void RobotWarsHumanView::SetControlledActorDelegate(IEventDataPtr pEventData)
{
...
void RobotWarsHumanView::DestroyPlayerDelegate(IEventDataPtr pEventData)
{
...
void RobotWarsHumanView::RegisterAllDelegates(void)
{
    ...
    pGlobalEventManager->VAddListener(MakeDelegate(this, &RobotWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &RobotWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VAddListener(MakeDelegate(this, &RobotWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);

}

void RobotWarsHumanView::RemoveAllDelegates(void)
{
    IEventManager* pGlobalEventManager = IEventManager::Get();
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RobotWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RobotWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RobotWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);
}
...
///////////////////////////////////////////////////////////////////////////////
//
// AIRobotView::AIRobotView					- Chapter 19, page 737
//
AIRobotView::AIRobotView(shared_ptr<PathingGraph> pPathingGraph) : IGameView(), m_pPathingGraph(pPathingGraph)
{
    //
}
...
// AIRobotView::~AIRobotView					- Chapter 19, page 737
//
AIRobotView::~AIRobotView(void)
{
    GCC_LOG("AI", "Destroying AIRobotView");
}
